...
def get_gtfs_dates_from_calendar(start_or_end_date_key, dataset_calendar, dates_per_service_id_dataframe):
    for index, row in dataset_calendar.iterrows():
        ...
        for day in WEEKDAYS:
            if row[day] == 1:
                service_id = row[SERVICE_ID]
                date_as_datetime = timedelta_operator(row_date_as_datetime, timedelta(days=day_offset_to_monday))
...
def get_gtfs_stop_times_for_date(dataset, dataset_dates, date_to_look_up):
    ...
    service_dates = dataset_dates.loc[dataset_dates[DATE] == date_to_look_up.strftime(DATE_FORMAT)]
