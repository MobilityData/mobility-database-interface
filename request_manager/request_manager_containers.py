...
class Clients(containers.DeclarativeContainer):
    ...
    staging_sparql_client = providers.Singleton(
        SparqlClient, Configs.staging_sparql_config
    )

    production_api_client = providers.Singleton(
        ApiClient, Configs.production_api_config
    )
    production_sparql_client = providers.Singleton(
        SparqlClient, Configs.production_sparql_config
    )
...
class Managers(containers.DeclarativeContainer):
    ...
    staging_api_request_manager = providers.Factory(
        ApiRequestManager, client=Clients.staging_api_client
    )
    staging_sparql_request_manager = providers.Factory(
        SparqlRequestManager, client=Clients.staging_sparql_client
    )

    production_api_request_manager = providers.Factory(
        ApiRequestManager, client=Clients.production_api_client
    )
    production_sparql_request_manager = providers.Factory(
        SparqlRequestManager, client=Clients.production_sparql_client
    )
