...
class TestProcessStartServiceDateForGtfsMetadata(TestCase):
    ...
    def test_process_start_service_date_with_dataset_with_missing_files(self):
    ...
    def test_process_start_service_date_with_dataset_with_feed_info(self):
        mock_feed_info = PropertyMock(
            return_value=pd.DataFrame({START_DATE_MAP[FEED_DATE_KEY]: ["20201010"]})
        )
        mock_dataset = MagicMock()
        mock_dataset.__class__ = Feed
        type(mock_dataset).feed_info = mock_feed_info
    ...
    @mock.patch("usecase.process_service_date_for_gtfs_metadata.get_gtfs_dates_by_type")
    def test_process_start_service_date_with_dataset_with_none_feed_info(
        self, mock_dates_by_type
    ):
    ...
    @mock.patch("usecase.process_service_date_for_gtfs_metadata.get_gtfs_dates_by_type")
    def test_process_start_service_date_with_dataset_with_none_feed_info_and_calendar_dates(
        self, mock_dates_by_type
    ):
        mock_feed_info = PropertyMock(return_value=None)
        mock_calendar = PropertyMock(
