...
class ProcessStartTimestampForGtfsMetadataTest(TestCase):
    ...
    def test_process_start_timestamp_with_none_gtfs_representation_should_raise_exception(self):
        self.assertRaises(TypeError, ProcessStartTimestampForGtfsMetadata, None)

    def test_process_start_timestamp_with_invalid_gtfs_representation_should_raise_exception(self):
        mock_gtfs_representation = MagicMock()
        mock_gtfs_representation.__class__ = str
        self.assertRaises(TypeError, ProcessStartTimestampForGtfsMetadata, mock_gtfs_representation)

    @mock.patch('representation.gtfs_representation.GtfsRepresentation')
    def test_process_start_timestamp_with_valid_gtfs_representation_should_return_instance(self,
                                                                                           mock_gtfs_representation):
        mock_gtfs_representation.__class__ = GtfsRepresentation
        under_test = ProcessStartTimestampForGtfsMetadata(mock_gtfs_representation)
        self.assertIsInstance(under_test, ProcessStartTimestampForGtfsMetadata)

    @mock.patch('representation.gtfs_representation.GtfsRepresentation')
    @mock.patch('gtfs_kit.feed.Feed')
    @mock.patch('representation.gtfs_metadata.GtfsMetadata')
    def test_process_start_timestamp_execution_should_set_start_timestamp_metadata(self, mock_gtfs_representation,
        ...
        under_test = ProcessStartTimestampForGtfsMetadata(mock_gtfs_representation).execute()
        ...
        mock_gtfs_representation.set_metadata_start_timestamp.assert_called_with('2020-10-10T05:00:00-05:00')
