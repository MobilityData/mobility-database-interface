...
class TestProcessEndServiceDateForGtfsMetadata(TestCase):
    ...
    def test_process_end_service_date_with_dataset_with_non_empty_feed_info_should_use_it(
        self, mock_gtfs_representation, mock_dataset, mock_metadata
    ):
        mock_feed_info = PropertyMock(
            return_value=pd.DataFrame({"feed_end_date": ["20201010"]})
        )
        ...
        mock_gtfs_representation.set_metadata_end_service_date.assert_called_with(
            "2020-10-10"
        )
    ...
    @mock.patch("gtfs_kit.feed.Feed")
    @mock.patch("representation.gtfs_metadata.GtfsMetadata")
    def test_process_end_service_date_with_dataset_with_empty_feed_info_should_use_calendars(
        self, mock_gtfs_representation, mock_dataset, mock_metadata
    ):
    ...
    @mock.patch("gtfs_kit.feed.Feed")
    @mock.patch("representation.gtfs_metadata.GtfsMetadata")
    def test_process_end_service_date_with_dataset_with_none_feed_info_should_use_calendars(
        self, mock_gtfs_representation, mock_dataset, mock_metadata
    ):
