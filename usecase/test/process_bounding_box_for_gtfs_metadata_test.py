...
class ProcessBoundingBoxForGtfsMetadataTest(TestCase):
    ...
    def test_process_bounding_box_with_none_gtfs_representation_should_raise_exception(self):
        self.assertRaises(TypeError, ProcessBoundingBoxForGtfsMetadata, None)

    def test_process_bounding_box_with_invalid_gtfs_representation_should_raise_exception(self):
        mock_gtfs_representation = MagicMock()
        mock_gtfs_representation.__class__ = str
        self.assertRaises(TypeError, ProcessBoundingBoxForGtfsMetadata, mock_gtfs_representation)

    @mock.patch('representation.gtfs_representation.GtfsRepresentation')
    def test_process_bounding_box_with_valid_gtfs_representation_should_return_instance(self,
    ...
    def test_process_bounding_box_execution_should_set_bounding_box_metadata(self, mock_gtfs_representation,
                                                                             mock_dataset, mock_metadata):
        mock_stops = PropertyMock(return_value=pd.DataFrame({'stop_lat': [45.508888], 'stop_lon': [-73.561668]}))
        ...
        under_test = ProcessBoundingBoxForGtfsMetadata(mock_gtfs_representation).execute()
        ...
        mock_gtfs_representation.set_metadata_bounding_box.assert_called_with({'1': '45°30\'31.997"N, 73°33\'42.005"W',
