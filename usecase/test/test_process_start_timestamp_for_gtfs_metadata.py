...
class TestProcessStartTimestampForGtfsMetadata(TestCase):
    def test_process_start_timestamp_with_none_gtfs_representation_should_raise_exception(
        self,
    ):
        self.assertRaises(TypeError, process_start_timestamp_for_gtfs_metadata, None)

    def test_process_start_timestamp_with_invalid_gtfs_representation_should_raise_exception(
        self,
    ):
    ...
    def test_process_start_timestamp_execution_missing_files(self):
            ...
            return_value=pd.DataFrame(
                {
                    ...
                    START_TIMESTAMP_MAP[STOP_TIME_KEY]: ["test_stop_time"],
                }
            )
        )
    ...
    def test_process_start_timestamp_execution_missing_stop_times_file(self):
        mock_calendar = PropertyMock(
            return_value=pd.DataFrame(
                {
                    START_TIMESTAMP_MAP[CALENDAR_DATE_KEY]: ["20201010"],
                    MONDAY: [0],
                    TUESDAY: [0],
                    WEDNESDAY: [0],
                    THURSDAY: [0],
    ...
    def test_process_start_timestamp_execution_with_no_calendar_dates(
        ...
        mock_stop_times_for_date.return_value = pd.DataFrame(
            {
                TRIP_ID: ["test_trip_id"],
                START_TIMESTAMP_MAP[STOP_TIME_KEY]: ["05:00:00"],
            }
        )
    ...
    def test_process_start_timestamp_execution_with_no_calendar(
        self, mock_stop_times_for_date, mock_utc_offset, mock_dates_by_type
    ):
