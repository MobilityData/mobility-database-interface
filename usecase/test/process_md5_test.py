...
class ProcessMd5Test(TestCase):
    ...
    def test_process_md5_with_invalid_paths_to_datasets_should_raise_exception(self):
        ...
        mock_md5_hashes = MagicMock()
        mock_md5_hashes.__class__ = dict
        self.assertRaises(TypeError, ProcessMd5, mock_paths_to_datasets, mock_md5_hashes)

    def test_process_md5_with_none_md5_hashes_should_raise_exception(self):
        mock_paths_to_datasets = MagicMock()
        mock_paths_to_datasets.__class__ = dict
        self.assertRaises(TypeError, ProcessMd5, mock_paths_to_datasets, None)
    ...
    def test_process_md5_with_valid_parameters_should_not_raise_exception(self):
        ...
        under_test = ProcessMd5(mock_paths_to_datasets, mock_md5_hashes)
        self.assertIsInstance(under_test, ProcessMd5)
        mock_paths_to_datasets.items.assert_not_called()
        mock_paths_to_datasets.keys.assert_called_once()
    ...
    def test_process_md5_with_dataset_md5_not_in_md5_hashes_should_return_dataset(self):
        ...
        test_paths_to_datasets_and_md5 = {'Q80': {'path': './usecase/test/resources/test.zip',
                                                  'md5': 'd41d8cd98f00b204e9800998ecf8427e'}}

        mock_paths_to_datasets = MagicMock()
        mock_paths_to_datasets.__class__ = dict
        mock_paths_to_datasets.__getitem__.side_effect = test_paths_to_datasets.__getitem__
        ...
        under_test = ProcessMd5(mock_paths_to_datasets, mock_md5_hashes).execute()
        self.assertEqual(under_test.items(), test_paths_to_datasets_and_md5.items())
        mock_paths_to_datasets.keys.assert_called()
        mock_md5_hashes.keys.assert_called_once()

    def test_process_md5_with_dataset_md5_existing_in_md5_hashes_should_discard_dataset(self):
        ...
        mock_paths_to_datasets.items.return_value = test_paths_to_datasets.items()
        mock_paths_to_datasets.keys.return_value = test_paths_to_datasets.keys()
        mock_paths_to_datasets.__delitem__.side_effect = test_paths_to_datasets.__delitem__
